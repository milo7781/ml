...
<html>
  ...
  <body>
	<form action="uzduotis.php" method="GET">
	...
	if (isset($_GET['siusti']) && $_GET['kiek']!=NULL && is_numeric($_GET['kiek'])) {
		$suma=$_GET['kiek'];
		$k1=floor($suma/500);
		$k2=floor(($suma-($k1*500))/100);
		$k3=floor(($suma-($k1*500)-($k2*100))/50);
		$k4=floor(($suma-($k1*500)-($k2*100)-($k3*50))/20);
		$k5=floor(($suma-($k1*500)-($k2*100)-($k3*50)-($k4*20))/10);
		$k6=floor(($suma-($k1*500)-($k2*100)-($k3*50)-($k4*20)-($k5*10))/5);
		$k7=floor(($suma-($k1*500)-($k2*100)-($k3*50)-($k4*20)-($k5*10)-($k6*5))/1);
